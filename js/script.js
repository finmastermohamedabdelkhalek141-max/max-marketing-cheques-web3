...
function numberToArabicWords(amount){
  ...
  const baisa = Math.floor((amount - rial) * 1000);
  ...
  if(client.installmentMode === "consultant"){
    ...
    rows.forEach((row, i)=>{
      ...
      installments.push({
        ...
        year:null, month:i+1, amount: amt,
...
function resolveYearPhases(yearVal, p1raw, p2raw, p3raw){
  ...
  const autoRate = remaining/(autoMonths+1);
  ...
  const lastInst = yearVal-flexTotal;
